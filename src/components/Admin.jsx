...
export default function Admin({ 
  ...
  return (
    <div className="flex h-screen bg-[#f8fafc] font-sans text-slate-900 overflow-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 z-20 w-64 bg-white border-r border-slate-200 flex flex-col transition-transform duration-300 md:relative md:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-16 flex items-center px-6 border-b border-slate-100 justify-between md:justify-start">
            <div className="flex items-center">
                <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center text-white font-black shadow-sm">JB</div>
                <div className="ml-3 font-bold text-slate-900 tracking-tight">Terminal Hub</div>
            </div>
        ...
        <nav className="p-4 space-y-1 flex-1 overflow-y-auto">
           ...
           <SidebarLink id="manifest" icon={Database} label="Logistics" active={activeTab} setActive={(id) => {setActiveTab(id); setSidebarOpen(false);}} />
           <SidebarLink id="invoices" icon={Receipt} label="Finance" active={activeTab} setActive={(id) => {setActiveTab(id); setSidebarOpen(false);}} />
           ...
           <SidebarLink id="vehicle" icon={Car} label="Fleet" active={activeTab} setActive={(id) => {setActiveTab(id); setSidebarOpen(false);}} />
           <SidebarLink id="settings" icon={Settings} label="Settings" active={activeTab} setActive={(id) => {setActiveTab(id); setSidebarOpen(false);}} />
        </nav>
        <div className="p-4 border-t border-slate-100"><button onClick={onLogout} className="w-full flex items-center gap-3 px-4 py-2 text-xs font-bold text-slate-500 hover:bg-red-50 hover:text-red-600 rounded-lg transition-colors"><LogOut size={16}/> Terminate Session</button></div>
      </aside>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden w-full relative">
         <div className="md:hidden h-16 bg-white border-b border-slate-200 flex items-center px-4 justify-between shrink-0">
             <span className="font-bold text-slate-900">Jay-Besin Control</span>
...
const SidebarLink = ({ id, icon: Icon, label, active, setActive, badge }) => (
  <button onClick={()=>setActive(id)} className={`w-full flex justify-between items-center px-4 py-2.5 rounded-lg text-xs font-bold transition-all ${active===id ? 'bg-blue-50 text-blue-600 shadow-sm' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'}`}>
    <div className="flex items-center gap-3"><Icon size={18} /> {label}</div>
    {badge > 0 && <span className="bg-blue-600 text-white px-2 py-0.5 rounded-full text-[9px] font-black">{badge}</span>}
  </button>
);
